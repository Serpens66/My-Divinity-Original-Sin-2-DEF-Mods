...
-- Ext.Print("Client Script Started Serp66 Mod SmallChanges_Serp")
...
Game.Tooltip.Register.Ability(function(char, ability, tooltip)
  ...
  elseif ability=="Leadership" then -- change range to new value
    for _, entry in ipairs(tooltip.Data) do
      if entry.Type=="AbilityDescription" then
        -- print("AbilityDescription",entry.Description,entry.Description:find(" 8 m ",1,true),entry.Description:find("8 m",1,true),entry.Description:find("8 m",1,true))
